    ...
    <div class="neckPanel" :style="{'box-shadow':' 0 0 5px 5px '+neck.quality.color + 'b8'}" v-if="JSON.stringify(neck)!='{}'">
      ...
      <div class="entry">
        <div v-for="v in neck.type.entry" :key="v.id">
          ...
          <div>{{v.name}} : {{v.showVal}} <span style="color:#68d5ed" v-if="neck.enchantlvl">(+{{Math.round(v.value*(1.05**(neck.enchantlvl)**1.1)-v.value)}})</span><span style="float:right">{{v.strength}}</span></div>
        </div>
      </div>
